...
#define GYRO_STARTUP_TIME                     50    // Start-up time max. 50 ms
#define QUAT_STARTUP_TIME                     50
...
static sensorCBs_t sensorTag_QuatCBs =
{
  quatChangeCB,             // Characteristic value change callback
};
...
uint16 SensorTag_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & ST_QUATERNION_SENSOR_EVT )
  {

     uint8 statusG;

    statusG = HalGyroStatus();
    Magnetometer_States_t statusM= HalMagStatus();
    
    if(gyroEnabled && magEnabled )
    {
      if (statusG == HAL_GYRO_STOPPED  )
      {
        HalGyroSelectAxes(0x07);
        HalGyroTurnOn();
        osal_start_timerEx( sensorTag_TaskID, ST_QUATERNION_SENSOR_EVT, QUAT_STARTUP_TIME);
      }
      else if (statusM == MAG3110_OFF )
      {
        HalMagTurnOn();
        osal_start_timerEx( sensorTag_TaskID, ST_QUATERNION_SENSOR_EVT, QUAT_STARTUP_TIME);
      }
      else
      {
        ...
        if (statusG == HAL_GYRO_DATA_READY && statusM==MAG3110_DATA_READY)
        {
          readQuatData();
          osal_start_timerEx( sensorTag_TaskID, ST_QUATERNION_SENSOR_EVT, sensorQuatPeriod);
        }
        else if (statusG != HAL_GYRO_DATA_READY)
        {
          // Gyro needs to be activated;
          HalGyroWakeUp();
          osal_start_timerEx( sensorTag_TaskID, ST_QUATERNION_SENSOR_EVT, QUAT_STARTUP_TIME);
    ...
    else
    {
      ...
      resetCharacteristicValue( QUATERNION_SERV_UUID, SENSOR_DATA, 0, QUATERNION_DATA_LEN);
      resetCharacteristicValue( QUATERNION_SERV_UUID, SENSOR_CONF, ST_CFG_SENSOR_DISABLE, sizeof( uint8 ));
    }

    return (events ^ ST_QUATERNION_SENSOR_EVT);
  }
  
  
  if ( events & ST_QUATERNION_UPDATE_EVT )
  {
    if (quatEnabled){
    updateQuatData();
    osal_start_timerEx( sensorTag_TaskID, ST_QUATERNION_UPDATE_EVT, 30);
    }
    return (events ^ ST_QUATERNION_UPDATE_EVT);
  }
...
static void updateQuatData( void )
{
   union Data qq0,qq1,qq2,qq3;
    ...
    Quat_SetParameter( SENSOR_DATA, 16, qData);
...
static void readQuatData( void )
{
  ...
  if (HalGyroRead(gData)&&HalMagRead(mData)&&HalAccRead(aData))
  {
...
static void readQuatDataIMU( void )
{
  ...
  if (HalGyroRead(gData)&&HalAccRead(aData))
  {
    float gx=calcXValueG(gData);
    float gy=calcYValueG(gData);
    float gz=calcZValueG(gData);
 ...
 * @fn      quatChangeCB
 *
 * @brief   Callback from QuatProfile indicating a value change
 *
 * @param   paramID - parameter ID of the value that was changed.
 *
 * @return  none
...
static void quatChangeCB( uint8 paramID )
{
  ...
  switch (paramID) {
   case SENSOR_CONF:
    Quat_GetParameter( SENSOR_CONF, &newValue );
    ...
    if (newValue == 0)
    {
     ...
     osal_set_event( sensorTag_TaskID, ST_QUATERNION_SENSOR_EVT);
    }
    else
    {
      // Bitmap tells which axis to enable (bit 0: X, but 1: Y, but 2: Z)
      ...
      sensorGyroUpdateAxes = TRUE;
      osal_set_event( sensorTag_TaskID,  ST_QUATERNION_SENSOR_EVT);
      osal_set_event( sensorTag_TaskID,  ST_QUATERNION_UPDATE_EVT);
    }
    break;
    
  case SENSOR_PERI:
    Quat_GetParameter( SENSOR_PERI, &newValue );
    sensorQuatPeriod = newValue*SENSOR_PERIOD_RESOLUTION;
    break;
...
static void resetCharacteristicValue(uint16 servUuid, uint8 paramID, uint8 value, uint8 paramLen)
{
  ...
  switch(servUuid)
  {
    ...
    case QUATERNION_SERV_UUID:
      Quat_SetParameter( paramID, paramLen, pData);
      break;
...
static void resetCharacteristicValues( void )
{
  ...
  resetCharacteristicValue( QUATERNION_SERV_UUID, SENSOR_DATA, 0, QUATERNION_DATA_LEN);
  resetCharacteristicValue( QUATERNION_SERV_UUID, SENSOR_CONF, ST_CFG_SENSOR_DISABLE, sizeof( uint8 ));
  resetCharacteristicValue( QUATERNION_SERV_UUID, SENSOR_PERI, QUAT_DEFAULT_PERIOD / SENSOR_PERIOD_RESOLUTION, sizeof ( uint8 ));
